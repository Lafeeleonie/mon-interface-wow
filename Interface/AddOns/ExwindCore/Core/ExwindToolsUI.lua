...
local THEME = {
    -- [v26.7 Style] Protocol 风格：以低对比深色画布承载信息，强调色仅用于状态和主操作。
...
function EXUI:CreateMainFrame()
    ...
    -- [v26.7 Style] 原创环境光纹理位于最底层；只提供空间深度，不参与交互也不干扰内容可读性。
    local ambient = f:CreateTexture(nil, "BACKGROUND")
    ambient:SetAllPoints()
    ambient:SetTexture(UI_AMBIENT_TEXTURE)
    ambient:SetAlpha(0.34)
    f.AmbientTexture = ambient

    local ambientMask = f:CreateTexture(nil, "BACKGROUND", nil, 1)
...
function EXUI:CreateSidebar(parent)
    ...
    sidebar:SetSize(272, 630)
    sidebar:SetPoint("TOPLEFT", 10, -50)
    sidebar:SetBackdrop(BACKDROP_SIMPLE)
    sidebar:SetBackdropColor(unpack(THEME.Sidebar))

    -- [Style] 添加一条垂直分割线，区分侧边栏和内容区
    local vLine = sidebar:CreateTexture(nil, "ARTWORK")
    vLine:SetSize(1, 620)
    vLine:SetPoint("TOPRIGHT", -2, 0)
    vLine:SetColorTexture(0.42, 0.38, 0.54, 0.28)

    local scrollFrame
    local searchBox = EXUI:CreateEditBox(sidebar, EXUI.SidebarState.SearchText or "", 248, 28, nil, {
        placeholder = L["搜索模块..."],
    ...
    if searchBox.SetBackdrop then
        searchBox:SetBackdrop(FRAME_BACKDROP_FLAT)
        searchBox:SetBackdropColor(0.055, 0.055, 0.08, 0.82)
        searchBox:SetBackdropBorderColor(0.30, 0.28, 0.38, 0.55)
    end
    EXUI.SidebarSearchBox = searchBox

    -- 为滚动条分配固定名称以启用材质隐藏逻辑
    scrollFrame = CreateFrame("ScrollFrame", "ExwindSidebarScroll", sidebar, "UIPanelScrollFrameTemplate")
...
local function ApplySidebarItemLayout(btn, variant)
    ...
    if btn.variant == "topnav" then
        ...
        btn.accent:Hide()
        btn.dot:Hide()
        btn.topnavAccent:Show()
        btn.label:ClearAllPoints()
        btn.label:SetPoint("LEFT", 12, 0)
        btn.label:SetWidth(214)
        btn.label:SetJustifyH("LEFT")
        btn.label:SetFont(defaultFontPath, 14, "")
    else
        btn:SetHeight(28)
        btn.rail:Show()
        btn.accent:Show()
        ...
        btn.label:SetWidth(190)
        btn.label:SetJustifyH("LEFT")
        btn.label:SetFont(defaultFontPath, 13, "")
    end
end
...
function EXUI:CreateCategoryHeaderBase(parent)
    ...
    btn.label:SetFont(defaultFontPath, 12, "")
    btn.label:SetPoint("LEFT", 0, 0)
    btn.label:SetJustifyH("LEFT")
    btn.label:SetTextColor(0.48, 0.45, 0.55, 1)

    -- 交互反馈
    btn:SetScript("OnEnter", function(self)
        self.label:SetTextColor(0.68, 0.64, 0.76, 1)
    end)
    btn:SetScript("OnLeave", function(self)
        self.label:SetTextColor(0.48, 0.45, 0.55, 1)
    end)
...
function EXUI:CreateSidebarItemBase(parent)
    ...
    btn.rail:SetWidth(1)
    btn.rail:SetColorTexture(0.30, 0.28, 0.37, 0.42)
    ...
    btn.accent:SetColorTexture(unpack(THEME.Primary))
    btn.accent:SetAlpha(0)

    btn.dot = btn:CreateFontString(nil, "OVERLAY")
    btn.dot:SetFont(defaultFontPath, 13, "OUTLINE")
    btn.dot:SetPoint("CENTER", btn, "LEFT", 10, 0)
    btn.dot:SetText("•")
    btn.dot:SetTextColor(THEME.Primary[1], THEME.Primary[2], THEME.Primary[3], 0.0)

    btn.topnavAccent = btn:CreateTexture(nil, "ARTWORK")
    ...
    btn.label:SetPoint("LEFT", 26, 0)
    btn.label:SetJustifyH("LEFT")
    btn.label:SetTextColor(0.63, 0.60, 0.69, 1)
    btn.label:SetWidth(190)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        if not self.isActive then
            if self.variant == "topnav" then
                local r, g, b = unpack(self.topnavColor or { 0.82, 0.86, 0.94 })
                self.label:SetTextColor(math.min(r * 1.08, 1), math.min(g * 1.08, 1), math.min(b * 1.08, 1), 1)
                self.topnavAccent:SetAlpha(0.95)
            else
                self.label:SetTextColor(0.87, 0.84, 0.94, 1)
                self.rail:SetColorTexture(0.48, 0.43, 0.62, 0.74)
                self:SetBackdropColor(0.12, 0.105, 0.17, 0.52)
            end
        end
    end)
    btn:SetScript("OnLeave", function(self)
        ...
        if not self.isActive then
            if self.variant == "topnav" then
                local r, g, b = unpack(self.topnavColor or { 0.82, 0.86, 0.94 })
                self.label:SetTextColor(r, g, b, 0.96)
                self.topnavAccent:SetAlpha(0.55)
            else
                self.label:SetTextColor(0.63, 0.60, 0.69, 1)
                self.rail:SetColorTexture(0.30, 0.28, 0.37, 0.42)
...
function EXUI:BuildNavigationTree(parent)
    ...
    local function CreateItem(name, page, key, meta, variant)
        ...
        yOffset = yOffset - (btn:GetHeight() or 28)
    end
    ...
    if isSearching and totalVisibleModules == 0 then
        ...
        emptyItem:SetPoint("TOPLEFT", 0, yOffset)
...
function EXUI:UpdateNavButtonStates()
    ...
    for _, btn in ipairs(EXUI.SidebarPool.Items) do
        if btn:IsShown() and btn.page then
            ...
            if isActive then
                if btn.variant == "topnav" then
                    local r, g, b = unpack(btn.topnavColor or { 0.82, 0.86, 0.94 })
                    btn.label:SetTextColor(r, g, b, 1)
                    btn.accent:SetAlpha(0)
                    btn.dot:SetTextColor(THEME.Primary[1], THEME.Primary[2], THEME.Primary[3], 0.0)
                    btn.topnavAccent:SetAlpha(1)
                else
                    btn.label:SetTextColor(0.88, 0.84, 0.98, 1)
                    btn.accent:SetAlpha(1)
                    btn.dot:SetTextColor(THEME.Primary[1], THEME.Primary[2], THEME.Primary[3], 1)
                    btn.rail:SetColorTexture(0.48, 0.43, 0.62, 0.7)
                    btn:SetBackdropColor(0.13, 0.11, 0.19, 0.68)
                end
            else
                btn.accent:SetAlpha(0)
                btn.dot:SetTextColor(THEME.Primary[1], THEME.Primary[2], THEME.Primary[3], 0.0)
                if btn.isLoaded == false then
                    btn.label:SetTextColor(0.38, 0.41, 0.48, 1)
                    if btn.variant ~= "topnav" then
                        btn.rail:SetColorTexture(0.20, 0.19, 0.25, 0.35)
                    end
                else
                    if btn.variant == "topnav" then
                        local r, g, b = unpack(btn.topnavColor or { 0.82, 0.86, 0.94 })
                        btn.label:SetTextColor(r, g, b, 0.96)
                        btn.topnavAccent:SetAlpha(0.55)
                    else
                        btn.label:SetTextColor(0.63, 0.60, 0.69, 1)
                        btn.rail:SetColorTexture(0.30, 0.28, 0.37, 0.42)
                    end
...
function EXUI:CreateRightPanel(parent)
    ...
    panel:SetSize(858, 630)
    panel:SetPoint("TOPLEFT", 318, -50)
...
function EXUI:ShowHomePage()
    ...
    local hasStandaloneExwindTools = ExwindTools.IsStandaloneExwindToolsLoaded and ExwindTools:IsStandaloneExwindToolsLoaded()
    local tipAnchor = localeStatus

    if hasStandaloneExwindTools then
        ...
        tipAnchor = minimapToggle
    else
        local minimapHint = Text(actionPanel, L["未检测到 ExwindTools 插件目录，小地图按钮已自动禁用。"], 11, HOME.muted,
            "TOPLEFT", localeStatus, "BOTTOMLEFT", 0, -18, INNER_W)
        minimapHint:SetWordWrap(true)
        tipAnchor = minimapHint
    end

    local tipHeader = Text(actionPanel, L["使用建议"], 13, HOME.gold, "TOPLEFT", tipAnchor, "BOTTOMLEFT", 0, -20, nil, "OUTLINE")
    local tips = {
...
function EXUI:ShowDiagnosticPage()
    ...
    local pageIntro = page:CreateFontString(nil, "OVERLAY")
    ...
    pageIntro:SetText("实时查看 ExwindTools 当前环境、玩家状态与核心运行信息。")
    ...
    AddSection("环境信息")
    AddFieldGrid({
        { label = "插件版本", value = ValueText(env.addonVersion) },
        { label = "WTF", value = ValueText(env.dbVersion) },
        { label = "游戏版本", value = ValueText(env.gameVersion) },
        { label = "Build", value = ValueText(env.gameBuild) },
        { label = "系统", value = ValueText(string.format("%s (%s)", env.platform, env.arch)) },
        { label = "区域", value = ValueText(env.region) },
        { label = "语言", value = ValueText(env.locale) },
        { label = "时间", value = ValueText(localTime) },
        { label = "PTR", value = env.isPTR == "是" and BoolText(true) or NO },
        { label = "BETA", value = env.isBeta == "是" and BoolText(true) or NO },
        { label = "ElvUI", value = env.isElvUI == "是" and BoolText(true) or NO },
    }, 2)

    AddSection("当前状态")
    AddFieldGrid({
        { label = "玩家", value = ValueText(SafeText(state.PlayerName, "?") .. "-" .. SafeText(state.RealmName, "?")) },
        { label = "职业", value = ValueText(SafeText(state.ClassName, "?")) },
        { label = "专精", value = ValueText(SafeText(state.SpecName, "?")) },
        { label = "职责", value = ValueText(SafeText(state.RoleName, "?")) },
        { label = "等级", value = ValueText(level) },
        { label = "战斗", value = BoolText(state.InCombat, "|cffff7a7a") },
        { label = "副本内", value = BoolText(state.InInstance) },
        { label = "五人本", value = BoolText(state.InFivePlayerInstance) },
        { label = "乘骑", value = BoolText(state.IsMounted) },
        { label = "开发模式", value = BoolText(state.DevMode, "|cffd9b3ff") },
        { label = "区域", value = ValueText(SafeText(state.ZoneText)), span = 2 },
        { label = "地图ID", value = ValueText(mapID) },
        { label = "地图组", value = ValueText(mapGroup) },
        { label = "副本ID", value = ValueText(instanceID) },
        { label = "副本类型", value = ValueText(SafeText(state.InstanceType, "none")) },
        { label = "难度ID", value = ValueText(SafeNum(state.DifficultyID)) },
        { label = "首领战", value = BoolText(state.IsBossEncounter, "|cffff7a7a") },
        { label = "EncounterID", value = ValueText(encounterID) },
        { label = "光环秘事", value = BoolText(state.AuraSecretsActive, "|cff7fd4ff") },
        { label = "打断就绪", value = BoolText(state.InterruptReady) },
    }, 2)

    AddSection("大秘境状态")
    AddFieldGrid({
        { label = "大秘境", value = BoolText(state.InMythicPlus, "|cff7fd4ff") },
        { label = "层数", value = ValueText(SafeNum(state.MythicPlusLevel)) },
        { label = "限时完成", value = BoolText(state.MythicPlusWasCharged) },
        { label = "词缀", value = ValueText(FormatList(state.MythicPlusAffixIDs)), span = 2 },
        { label = "敌方进度", value = ValueText(string.format("%d / %d", SafeNum(state.MythicPlusForcesCurrent), SafeNum(state.MythicPlusForcesTotal))) },
        { label = "百分比", value = ValueText(FormatPercent(state.MythicPlusForcesPercent)) },
        { label = "有效", value = BoolText(state.MythicPlusForcesValid) },
        { label = "进度文本", value = ValueText(SafeText(state.MythicPlusForcesText)), span = 2 },
        { label = "已击杀首领", value = ValueText(SafeNum(state.DungeonBossKilledCount)) },
        { label = "进度序号", value = ValueText(SafeNum(state.DungeonBossProgressIndex)) },
    }, 2)

    AddSection("玩家属性")
    AddFieldGrid({
        { label = "力量", value = ValueText(SafeNum(state.PStat_Str)) },
        { label = "敏捷", value = ValueText(SafeNum(state.PStat_Agi)) },
        { label = "智力", value = ValueText(SafeNum(state.PStat_Int)) },
        { label = "耐力", value = ValueText(SafeNum(state.PStat_Sta)) },
        { label = "主属性", value = ValueText(SafeNum(state.PStat_Major)) },
        { label = "暴击", value = ValueText(FormatPercent(state.PStat_Crit)) },
        { label = "急速", value = ValueText(FormatPercent(state.PStat_Haste)) },
        { label = "精通", value = ValueText(FormatPercent(state.PStat_Mastery)) },
        { label = "全能", value = ValueText(FormatPercent(state.PStat_Versa)) },
        { label = "护甲", value = ValueText(FormatLargeNumber(state.PStat_Armor)) },
        { label = "闪避", value = ValueText(FormatPercent(state.PStat_Dodge)) },
        { label = "招架", value = ValueText(FormatPercent(state.PStat_Parry)) },
        { label = "格挡", value = ValueText(FormatPercent(state.PStat_Block)) },
        { label = "吸血", value = ValueText(FormatPercent(state.PStat_Leech)) },
        { label = "减伤", value = ValueText(FormatPercent(state.PStat_Avoidance)) },
        { label = "速度", value = ValueText(FormatPercent(state.PStat_Speed)) },
        { label = "移速", value = ValueText(SafeText(state.PStat_MovementText, FormatPercent(state.PStat_Movement))) },
        { label = "装等", value = ValueText(FormatLargeNumber(state.PStat_EquippedItemLevel)) },
        { label = "最大生命", value = ValueText(FormatLargeNumber(state.PStat_MaxHealth)) },
        { label = "耐久", value = ValueText(FormatPercent(state.PStat_Durability)) },
    }, 2)
    ...
    AddFieldGrid({
        ...
        { label = "影遁可用", value = BoolText(state.ShadowmeldAvailable) },
        { label = "影遁冷却", value = BoolText(state.ShadowmeldCD, "|cffff7a7a") },
        { label = "到期时间", value = FormatSessionTime(state.ShadowmeldExpiration) },
    }, 2)
