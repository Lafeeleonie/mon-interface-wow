...
ExwindTools_MetaData = {
    version = "v26.7.2.1953",
    gridEngineVersion = "2.0",
    changelog = {
        title = "v26.6.24.0200 更新日志",
...
@H2@ Raid Marker Panel
- Support changing transparency on mouse hover
- Support swapping the positions of the Ready Check and Countdown buttons

@H1@ v26.6.20.0313

@H1@ 新增团队标记/光柱面板

@H1@ Added translations for the raid marker/world marker panel

@H1@ v26.6.17.1731

@H2@ 支援 12.0.7版本
- 修复史诗弹性难度下自动战斗日志不生效问题

@H2@ 焦点施法
- 现在每次登陆后都会锁定焦点施法 (避免有些人忘了锁定导致战斗中无法鼠标穿透)

@H2@ 开发计划
- 我们准备在这段时间逐渐更新大多数功能和GUI

@H2@ Support 12.0.7
- Fixed an issue where Auto Combat Log did not work in Mythic Flexible raids.

@H2@ Focus Cast
- Focus Cast is now locked by default on every login to prevent mouse interaction issues in combat when users forget to lock it.

@H2@ Development Plans
- We plan to gradually update most features and the GUI over time.

@H1@ v26.5.4.1416

@H2@ 编辑模式系统 (Edit Mode System)
- 所有勾选状态现在可以保存
- All checkbox states are now saved.
- 修复了许多拖动后不保存的问题
- Fixed many cases where positions were not saved after dragging.
- 所有ExwindTools的组件均接入新版编辑模式系统 如有问题请回报
- All ExwindTools components are now integrated into the new edit mode system. Please report any issues.

@H2@ 周围怪物施法 (Nearby Enemy Cast Monitor)
- 新增了屏蔽首领(92级怪物)的选项
- Added an option to hide boss casts (level 92 enemies).
- 如勾选以上选项 "不会屏蔽" 首领召唤的小怪
- If this option is enabled, it will not hide adds summoned by the boss.

@H1@ v26.5.3.0737

@H2@ 玩家属性显示 (Player Stats)
- 现在大秘境/团本内 能正常显示全能和移动速度
