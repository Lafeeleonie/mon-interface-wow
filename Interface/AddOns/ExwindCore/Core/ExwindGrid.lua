...
function Grid:ReleaseWidgetInstance(widget)
    ...
    if renderer and type(renderer.release) == "function" then
        pcall(renderer.release, widget, widget._customContext)
    end
...
function Grid:CreateWidget(container, ele, config, moduleKey, contextPath)
    ...
    elseif ele.type == "card" then
        ...
        if widget.SetBackdrop then
            widget:SetBackdrop({
                ...
                edgeSize = tonumber(ele.borderSize) or 1,
                insets = { left = 1, right = 1, top = 1, bottom = 1 },
            })
            widget:SetBackdropColor(bgR, bgG, bgB, bgA)
            widget:SetBackdropBorderColor(borderR, borderG, borderB, borderA)
        end
        ...
        if widget.Title then
            ...
            widget.Title:SetPoint("TOPLEFT", padding, -padding)
        ...
        if widget.Desc then
            ...
            if widget.Title then
                ...
                widget.Desc:SetPoint("TOPLEFT", widget, "TOPLEFT", padding, -padding)
            end
    ...
    elseif ele.type == "custom" then
        ...
        if renderer then
            ...
            if widget._customRendererKey ~= rendererKey and widget._customRenderer and type(widget._customRenderer.release) == "function" then
                pcall(widget._customRenderer.release, widget, widget._customContext)
            end
            ...
            if type(renderer.mount) == "function" then
                pcall(renderer.mount, widget, ctx)
            end
        else
    ...
    elseif ele.type == "fontgroup_v10" or ele.type == "fontgroupv10" then
        if not curVal then
            local defaultFontTable = {
                enabled = true,
        ...
        widget = EXUI:CreateFontGroupV10(container, pw, ele.label, curVal, function()
            if moduleKey then
                ExwindTools:UpdateState(moduleKey .. ".DatabaseChanged",
                    { key = ele.key, fullPath = fullPath, ts = GetTime() })
            end
    ...
    if widget then
        ...
        if ele.type == "custom" and widget._customRenderer then
            ...
            if type(widget._customRenderer.update) == "function" then
                pcall(widget._customRenderer.update, widget, widget._customContext)
            elseif type(widget._customRenderer.render) == "function" then
                pcall(widget._customRenderer.render, widget, widget._customContext)
            end
        end
...
function Grid:ShowPalette()
    ...
    local types = {
        ...
        { t = "label", n = "文本" }, { t = "description", n = "描述" }, { t = "card", n = "卡片" }, { t = "custom", n = "自定义宿主" }, { t = "color", n = "颜色" },
        ...
        { t = "fontgroup", n = "字体组" }, { t = "fontgroup_v10", n = "字体组V10" }
    }
    local y = -15
...
function Grid:AddNewWidget(t, c)
    ...
    elseif t:find("header") or t == "divider" then
        w, h = 47, 1
    elseif t == "card" or t == "custom" then
        w, h = 24, 8
    elseif t == "fontgroup" then
        w, h =
            47, 10
    elseif t == "fontgroup_v10" or t == "fontgroupv10" then
        w, h = 47, 36
    end
    ...
    for i = 1, 200 do
        if t == "card" or Grid:IsAreaEmpty(1, i, w, h) then
