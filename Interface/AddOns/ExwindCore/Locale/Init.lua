...
local function GetFallbackLocales(locale)
    locale = NormalizeLocaleTag(locale)
...
Locale._proxy = Locale._proxy or setmetatable({}, {
    __index = function(_, key)
        ...
        local currentLocale = Locale._currentLocale
        local cur = Locale._stores[currentLocale]
        if type(cur) == "table" and cur[key] ~= nil then return cur[key] end

        for _, fallbackLocale in ipairs(GetFallbackLocales(currentLocale)) do
            local fallbackStore = Locale._stores[fallbackLocale]
            if type(fallbackStore) == "table" and fallbackStore[key] ~= nil then
