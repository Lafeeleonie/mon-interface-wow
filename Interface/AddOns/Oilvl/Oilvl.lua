...
function oilvlcheckunknown()
	if not issecurevariable("UnitGUID") then return end
	if IsInRaid() then
		...
		if OilvlTooltip:IsShown() then
			...
			for i = 2, OilvlTooltip:NumLines() do
				...
				if msg then
					...
					if cfg.raidmenuid == 4 then msg = msg:find(CNname4); if msg then break end end
				end
			end
			if not msg then
	...
	for b4j = 1, 8 do
		for b4i = 1, 5 do
			...
			button4:SetScript("OnEnter", function(self)
				...
				if UnitAffectingCombat("player")  or UnitAffectingCombat("target") then return end
				if not otooltip2 then
...
function OGetRaidProgression2(RaidName, OSTAT, NumRaidBosses, Faction)
	...
	local function DrawOTooltip2()
		...
		otooltip2:SetCell(4,4,"|cffffffff"..CNname4,"LEFT",2)
		otooltip2:SetCellScript(4,4,"OnMouseUp",function(s)
			Save_orp_vars(CNname4)
			otooltip2:Clear()
			DrawOTooltip2()
		end)
...
function events:PLAYER_LOGIN(...)
	...
	if cfg.oilvlshowcurrentcharilvl == nil then cfg.oilvlshowcurrentcharilvl = true; end
	OilvlConfigFrame();
	oilvlframe();
	OVILRefresh();
	if minimapicon then
	...
	if cfg.oilvlms and TooltipDataProcessor then
		TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Unit, function(GameTooltip, data)
			if issecretvalue(UnitExists("target")) then return false end
			if issecretvalue(GetUnitName("target","")) then return false end
			--local _, unit = GameTooltip:GetUnit()
			
			local success, _, unit = pcall(unsafeGetUnit)
			if success == nil then return false end
			if not success then return false end
            if issecretvalue(unit) then return false end
			if unit ~= "target" then return end
			if issecretvalue(UnitIsUnit(unit, "target")) then return false end
			if issecretvalue(UnitIsUnit(unit, "player")) then return false end
			if issecretvalue(UnitIsUnit(unit, "mouseover")) then return false end
...
--  oilvl opacity
	...
	-- minimap icon option	
local micon = createCheckbutton(cfg.frame, 16, -440, "oilvlshowminimap", L["Show minimap button"]);
	micon:SetSize(30,30);
	...
	local printload = createCheckbutton(cfg.frame, 16, -500, "oilvlprintloaded", L["Print Loaded Message"]);
	...
	-- set current character item ilvl
	local showcurrentcharilvl = createCheckbutton(cfg.frame, 16+25, -410, "oilvlshowcurrentcharilvl", L["Show iLevel for each item on your character's screen"]);
	showcurrentcharilvl:SetSize(30,30);
	showcurrentcharilvl:SetScript("PostClick", function()
		cfg.oilvlshowcurrentcharilvl = showcurrentcharilvl:GetChecked()
	end);
	showcurrentcharilvl:SetChecked(cfg.oilvlshowcurrentcharilvl);

	-- item level decimal places
	local dptitle = cfg.frame:CreateFontString(nil,"ARTWORK","GameFontNormal")
	dptitle:SetTextColor(1,1,1)
	dptitle:SetPoint("TOPLEFT",16,-480)
	...
	dp:SetWidth(15)
	dp:SetHeight(20)
	dp:SetPoint("TOPLEFT",20+dptitle:GetStringWidth(),-477)
